...
export default function IndexPage() {
    ...
    useEffect(() => {
            getTargetMessageHistory(activeBot.activeMessageID ).then(res => {
            if (res) {
                resetList(res.list);
            }
        })
    ...
    function renderMessageContent(msg: any) {
        ...
        if (msg.role == 'user') {
            return (
                ...
                <Bubble onMouseDown={(e) => e.stopPropagation()} content={content.text} />
            );
        } else {
            switch (type) {
                case 'text':
    ...
    const updataLastMessage = (message:any) => {
        botState.lastMessage = message.content.text ? message.content.text :'[picture]'
        botState.lastData = message.createdAt
    }
