...
const App: React.FC = () => {
  const dataSource = useSnapshot(logitBiasState.logitBiasArray);
  ...
  const changeTab = async (id: string) => {
    ...
    // console.log(logitBiastate, 66666666);
    if (logitBiastate) {
      logitBiasStore.logitBiasState.logitBiasArray.splice(
        0,
        logitBiasStore.logitBiasState.logitBiasArray.length,
  ...
  const removeTab = async (id: string) => {
     await deleteTargetLogitBias(id, activeProject.activeProjectID);
  };
